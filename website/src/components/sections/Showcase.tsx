                ...
                <div className="flex">
                  <div className="w-48 bg-drift-surface p-3 border-r border-drift-border min-h-[400px]">
                    <div className="space-y-1">
                      {["Home", "Mods", "Cosmetics", "Profiles", "Account", "Settings"].map((item, i) => (
                        <div
                          key={item}
                          className={`px-3 py-2.5 rounded-lg text-sm ${
                            i === 0
                              ? "bg-drift-border text-drift-text"
                              : "text-drift-muted"
                          }`}
                        >
                          {item}
...
function Stat({ icon: Icon, label, value }: { icon: any; label: string; value: string }) {
  return (
    <div className="flex items-center gap-4">
      <div className="w-10 h-10 rounded-lg bg-drift-surface border border-drift-border flex items-center justify-center">
